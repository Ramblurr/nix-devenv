...
<a id="outline-container-enabling"></a>

<a id="enabling"></a>

## Enabling HA

<a id="text-enabling"></a>
...
<a id="outline-container-peer-recovery-time"></a>

<a id="peer-recovery-time"></a>

## Peer Recovery Time

<a id="text-peer-recovery-time"></a>
...
<a id="outline-container-reads-remain"></a>

<a id="reads-remain"></a>

## Reads Remain Available

<a id="text-reads-remain"></a>

Peers can continue to read and query during a transactor failover. In particular, calls to [datomic.api/db](../../../04-apis/01-peer-api-clojuredoc/peer-api-clojuredoc.md#datomic.api/db) will neither block nor fail during the window of a transactor failover.

<a id="outline-container-transaction-latency"></a>

<a id="transaction-latency"></a>

## Transaction Latency After HA Recovery

<a id="text-transaction-latency"></a>
...
<a id="outline-container-retry"></a>
...
Datomic does not automatically retry transactions after a transactor failure. Applications, aware of their own semantic needs, can implement an appropriate retry policy, with full access (e.g. via the [Log API](../../../04-apis/09-log-api/log-api.md)) to recent transaction history.

<a id="outline-container-switching-transactor-version"></a>

<a id="switching-transactor-version"></a>

## Switching to a Different Transactor Version

<a id="text-switching-transactor-version"></a>
...
<a id="outline-container-moving-across-data-centers"></a>

<a id="moving-across-data-centers"></a>

## Moving Across Data Centers / Regions

<a id="text-moving-across-data-centers"></a>
...
<a id="outline-container-use-datomic-backup"></a>

<a id="use-datomic-backup"></a>

### Use Datomic Backup For Consistent Copy

<a id="text-use-datomic-backup"></a>

Datomic's [backup](../08-backup-and-restore/backup-and-restore.md) makes a *consistent copy* of Datomic data, i.e. backup preserves Datomic's consistency guarantees. Thus Datomic backup is suitable for use in a disaster-recovery plan.

<a id="outline-container-other-consistent-copy-options"></a>

<a id="other-consistent-copy-options"></a>

### Other Consistent Copy Options

<a id="text-other-consistent-copy-options"></a>

Datomic treats [storage](../01-storage-services/storage-services.md) as a pluggable component, which raises the possibility of copying Datomic data using the underlying storage engine's backup or replication facilities, an approach hereafter called "storage copy". Storage copy may be preferable to Datomic backup for disaster recovery for reasons including:
