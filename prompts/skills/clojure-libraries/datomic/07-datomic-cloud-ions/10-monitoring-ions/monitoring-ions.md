...
AWS CloudWatch provides a powerful set of tools for monitoring a software system running on AWS, and Datomic [fully integrates with these tools](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md).
...
<a id="outline-container-events"></a>
...
When the ion code is running on Datomic Cloud, events are posted to [Datomic's CloudWatch log](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md). Datomic transforms data to [searchable JSON](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md) as follows:
...
<a id="outline-container-alerts"></a>
...
When the ion code is running on Datomic Cloud, alerts are posted to [Datomic's CloudWatch log](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md). The data transformation is the same as for [events](#events), except that the value for `Type` is "Alert".

In addition to the log entry, alerts create a [Datomic CloudWatch metric](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md) value named "Alerts. You can use the occurrence of this metric to e.g. send an SNS message to an operator.
...
Choose metric names that do not collide with Datomic's [built-in metric names](../../05-operation/01-pro/06-monitoring-and-performance/monitoring-and-performance.md).

<a id="outline-container-dev"></a>
...
<a id="outline-container-local-workflow"></a>

<a id="local-workflow"></a>

## Local Workflow

<a id="text-local-workflow"></a>

You may find it useful to see monitoring data when you are developing on a machine outside Datomic. You can call `initialize-redirect` once per process to all `alert`, `event`, and `dev` output to one of the following targets:
...
<a id="outline-container-java-logging"></a>

<a id="java-logging"></a>

## Java Logging

<a id="text-java-logging"></a>
