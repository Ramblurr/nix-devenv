...
- **Frontmatter** (YAML): Contains `name` and `description` fields. These are the only fields that the agent reads to determine when the skill gets used, so be clear and comprehensive in describing what the skill is and when it should be used.
...
3. **Bundled resources** - As needed by the agent (Unlimited because scripts can be executed without reading into the context window)
...
- Creates resource directories only for the types explicitly listed in `--resources`
...
If you used `--examples`, delete any placeholder files that are not needed for the skill. Resource directories must only be created when they will actually be used; do not leave empty `scripts/`, `references/`, or `assets/` directories behind.
